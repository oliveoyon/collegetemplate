                          ...
                          <table class="table table-hover table-condensed" id="menu-table">
                              ...
                              <tbody>
                                ...
                                <tr>
                                  ...
                                  <td class="{{ $menu->menu_status == 1 ? 'text-success' : 'text-danger' }} font-weight-bold">
                                    {{ $menu->menu_status == 1 ? 'একটিভ' : 'একটিভ নয়' }}
                                  </td>
                                  <td>
                                    <span data-id="{{ $menu->id }}" id="editMenuBtn"><i class="fas fa-edit text-warning"></i></span> &nbsp;
                                    <span data-id="{{ $menu->id }}" id="deleteMenuBtn"><i class="fas fa-trash-alt text-danger"></span></i>
                                  </td>
                                </tr>
                                @endforeach
...
$.ajaxSetup({
  ...
  $(document).on('click','#deleteMenuBtn', function(){
      ...
      swal.fire({
            ...
            if(result.value){
                $.post(url,{menu_id:menu_id}, function(data){
                      if(data.code == 1){
                          var redirectUrl = data.redirect;
                          toastr.success(data.msg);
                          setTimeout(function() {
