    ...
    <div class="content-wrapper">
        ...
        <div class="content">
            <div class="container-fluid">
                                    ...
                                    <ul class="nav nav-pills ml-auto">
                                        <li class="nav-item">
                                            ...
                                            <button class="btn btn-flat btn-success" data-toggle="modal"
                                                data-target="#addmessages"><i class="fas fa-plus-square mr-1"></i> বাণী যোগ
                                                করুন</button>
                                        </li>
                                    </ul>
                                </div>
                                    ...
                                    <tbody>
                                        @foreach ($messages as $message)
                                            <tr>
                                                ...
                                                <td>{{ $message->faculty_department }}</td>
                                                ...
                                                <td>
                                                    <img height="50"
                                                        src="{{ asset('storage/img/message_img/' . $message->upload) }}"
                                                        alt="">
                                                </td>
                                                <td
                                                    class="{{ $message->message_status == 1 ? 'text-success' : 'text-danger' }} font-weight-bold">
                                                    {{ $message->message_status == 1 ? 'একটিভ' : 'একটিভ নয়' }}
                                                </td>
                                                <td>
                                                    <span data-id="{{ $message->id }}" id="editMessageBtn"><i
                                                            class="fas fa-edit text-warning"></i></span> &nbsp;
                                                    <span data-id="{{ $message->id }}" id="deleteMessageBtn"><i
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-xl">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form action="{{ route('admin.addMessage') }}" enctype="multipart/form-data" files="true"
                                    method="post" autocomplete="off" id="add-message-form">
                                    @csrf
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="message_type" id="message_type"
                                            placeholder="পদবী">
                ...
                {{-- Edit Modal --}}
                <div class="modal fade editMessage" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form action="{{ route('admin.updateMessageDetails') }}" enctype="multipart/form-data"
                                    files="true" method="post" autocomplete="off" id="update-message-form">
                                    @csrf
...
@push('adminjs')
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.code == 0) {
                toastr.error(data.msg); // Show toastr error message
            } else {
            ...
            $(document).on('click', '#editMessageBtn', function() {
                ...
                $.post("{{ route('admin.getMessageDetails') }}", {
                    message_id: message_id
                }, function(data) {
                    //alert(data.details.version_name);
                    var messageModal = $('.editMessage');
                    $('.editMessage').find('input[name="sid"]').val(data.details.id);
                    $('.editMessage').find('input[name="message_type"]').val(data.details
                        .message_type);
                    $('.editMessage').find('input[name="name"]').val(data.details.name);
                    $('.editMessage').find('select[name="message_status"]').val(data.details
                        .message_status);
                    $('.editMessage').find('select[name="dept_id"]').val(data.details.dept_id);
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.code == 0) {
                // Display toastr error message if validation fails
                ...
                // Redirect and display success toastr message if update is successful
                var redirectUrl = data.redirect;
