                    ...
                    <article class="entry">
                        ...
                        @if ($childmenudesc->upload)
                            @if (pathinfo($childmenudesc->upload, PATHINFO_EXTENSION) === 'pdf')
                                ...
                                <object data="{{ asset('storage/img/childmenu_img/' . $childmenudesc->upload) }}"
                                    type="application/pdf" width="100%" height="800">
                                    <p>Unable to display PDF file. <a
                                            href="{{ asset('storage/img/childmenu_img/' . $childmenudesc->upload) }}">Download</a>
                                        instead.</p>
                                </object>
                            @else
                                <div class="entry-img">
                                    <img src="{{ asset('storage/img/childmenu_img/' . $childmenudesc->upload) }}"
                                        alt="" class="img-fluid">
                                </div>
                                <h2 class="entry-title">
                                    <a href="">{{ $childmenudesc->childmenu_name }}</a>
